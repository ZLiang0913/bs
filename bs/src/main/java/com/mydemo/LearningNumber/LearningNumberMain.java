...
public class LearningNumberMain {

	public static void main(String[] args) throws Exception {
		String[] path = {"hdfs://192.168.203.111:9000/sqoop/zt/zt_stu_study_schedule/part-m-00000",
        "hdfs://192.168.203.111:9000/sqoop/result/LearningNumber/output1"};
		System.setProperty("hadoop.home.dir", "C:/hadoop");
		
		Configuration config = new Configuration();
		//生成job实例，用户对整个分布式运算的配置
		Job job = Job.getInstance(config);
		job.setJarByClass(LearningNumberMain.class);
		
		//指定在本任务里，map和reduce文件是什么
		job.setMapperClass(LearningNumberMapper.class);
		job.setReducerClass(LearningNumberReducer.class);
